...
class _CalendarPageState extends State<CalendarPage> {
  ...
  void _fetchTeams() async {
    ...
    if (adminId != null) {
      final teamsSnapshot = await FirebaseFirestore.instance.collection(
          'ekipler').where('adminId', isEqualTo: adminId).get();
      setState(() {
  ...
  void _fetchPublicHolidays() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.googleapis.com/calendar/v3/calendars/tr.turkish%23holiday@group.v.calendar.google.com/events?key=AIzaSyDeqhvkAhLeU8tLg86SsnfrElerT0RgbK4')); // Google Calendar API URL'si
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        List<
            dynamic> holidays = data['items']; // 'items' kısmındaki verileri al
        setState(() {
          _publicHolidays = holidays.map((holiday) => holiday['start']['date'])
              .toList(); // Tarihleri listeye ekle
        });
      } else {
    ...
    try {
      ...
      if (adminId != null) {
        final eventsSnapshot = await FirebaseFirestore.instance
            ...
            .where(
            'date', isGreaterThanOrEqualTo: now) // Gelecekteki etkinlikler
            .orderBy('date')
            .get();
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text('Ekip Seçin:'),
                            Icon(
                              _isDropdownOpen ? Icons.arrow_drop_up : Icons
                                  .arrow_drop_down,
                              color: Colors.black,
                            ),
                          ],
                  ...
                  onPressed: () async {
                    if (_eventTitleController.text.isNotEmpty &&
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text(
                            'Lütfen tüm alanları doldurun ve ekip seçin.'),
                      ));
                    }
                  },
                  child: Text('Ekle'),
  ...
  void _showEditDialog(QueryDocumentSnapshot event) async {
    ...
    //ekipleri admin ıdye göre al
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? adminId = prefs.getString('adminId');
    List<String> teams = await _fetchTeamsByAdminId(adminId);
  ...
  void _updateEvent(String eventId, String title, String description,
      List<String> selectedTeams) async {
  ...
  Future<void> _addEvent(DateTime date, String title, String description,
      List<String> selectedTeams) async {
    try {
      ...
      for (String team in selectedTeams) {
        ...
        final teamSnapshot = await FirebaseFirestore.instance.collection(
            'ekipler').where('ad', isEqualTo: team).get();
      ...
      await FirebaseFirestore.instance.collection('events').add({
        ...
        // DateTime'i Timestamp'e çeviriyoruz
        'invitedTeams': selectedTeams,
        'teamsIds': teamsIds,
        //secilen ekiplerin teamsIdslerini ekle
        'adminId': adminId,
        //admin Idyi ekle
      });

      // Seçilen ekiplerin cihaz tokenlarını topla
      List<String> deviceTokens = await _fetchDeviceTokensForTeams(
          selectedTeams);
          ...
          TableCalendar(
            ...
            calendarBuilders: CalendarBuilders(
              defaultBuilder: (context, day, focusedDay) {
                final isHoliday = _publicHolidays.contains(
                  day
                      .toIso8601String()
                      .split('T')
                      .first,
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                      '${eventDate.toLocal()} - ${event['description']}'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      IconButton(
